...
variants. `src/profiles/<name>.py` is `[MY]` — everything else lives
...
| Source-LZ file names + roles                        | MY     | `profiles/*.py`|
| Accelerator prefix, home region, ASN default        | MY     | `profiles/*.py`|
| Account naming convention                           | MY     | `profiles/*.py`|
| Default OU targets for policies                     | MY     | `profiles/*.py`|
| Regional pinning / partition                        | MY     | `profiles/*.py`|
...
## Common failure modes (learned from Malaysia conversion)
...
generic?" If yes, `[GEN]`. If it's a customer/source choice, `[MY]`.
...
- ❌ Attempt mechanical translation of network routing detail without
   ...
   defer instead.
- ❌ Add Malaysia strings to `[GEN]` files. Push to profile.
...
For Malaysia: converged at 43 (28 network + 12 accounts + 3 customizations),
all matching enumerated stubs.
