...
├── profiles/
│   ├── malaysia.py      [MY]  Malaysia SLZ constants
│   ├── _template.py     [GEN] template for new profiles
│   └── <yours>.py       [MY]  new profile you add
└── mappers/
...
1. Clone your source LZ repo alongside `sample-malaysia-secure-lz/`:
   ```
   /path/to/workdir/
   ├── converter/
   ├── sample-malaysia-secure-lz/
   └── <your-source-lz>/            ← new
   ```
2. Copy the template:
   ```
...
a source-LZ convention or customer choice, it's `[MY]`.
